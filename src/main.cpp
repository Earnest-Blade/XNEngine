...
/*
    xne_create_shader(&state.shader, "assets/shader.glsl", shaders);
    xne_shader_link_uniforms(&state.shader, uniforms);

    xne_create_texture(&state.texture, "assets/texture.png", XNE_TEXTURE_FILTER_LINEAR, XNE_TEXTURE_WRAP_REPEAT);
    xne_link_texture(&state.texture, "texture0", state.shader.program);

    xne_create_plane(&state.mesh, 50, 50);
*/
    ...
    //xne_create_sprited(&state.sprite, sprite);

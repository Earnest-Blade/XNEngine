...
static void xne__model_create_node(json_object* json, xne_Model_t* model, xne_Model_t::xne_Model_Node* node){
    assert(json);
    assert(model);
    assert(node);
    ...
    xne_create_fixed_vector(&node->childs, sizeof(xne_Model_t::xne_Model_Node), json_object_array_length(childs));
    ...
    for (size_t i = 0; i < node->childs.count; i++)
    {
        xne__model_create_node(
            ...
            model,
            (xne_Model_t::xne_Model_Node*) xne_vector_get(&node->childs, i)
        );
    }
}

static void xne__model_draw_node(xne_Model_t::xne_Model_Node* node){
    if(!node) return;

    for (size_t i = 0; i < node->childs.count; i++)
    {
        xne__model_draw_node((xne_Model_t::xne_Model_Node*) xne_vector_get(&node->childs, i));
    }
...
static void xne__model_destroy_node(xne_Model_t::xne_Model_Node* node){
    if(!node) return;

    for (size_t i = 0; i < node->childs.count; i++)
    {
        xne__model_destroy_node((xne_Model_t::xne_Model_Node*)xne_vector_get(&node->childs, i));
    }

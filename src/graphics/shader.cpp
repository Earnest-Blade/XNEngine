...
int xne_create_shaderf(xne_Shader_t* shader, FILE* file, const xne_ShaderDesc_t* desc){
    ...
    shader->count = 0;
    xne_ShaderDesc_t* shr = const_cast<xne_ShaderDesc_t*>(&desc[0]);
    while (shr->type != 0)
        shr = const_cast<xne_ShaderDesc_t*>(&desc[++shader->count]);
    ...
    shader->uniforms = (xne_Shader_t::xne_ShaderUniform*) malloc(0);
    ...
    for (size_t i = 0; i < shader->count; i++)
    {
        ...
        success = xne__compile_shader(&shader->shaders[i], desc[i].type, cpy.c_str());
...
int xne_link_shader_uniforms(xne_Shader_t* shader, const xne_ShaderUniformDesc_t* uniforms){
    ...
    xne_ShaderUniformDesc_t* uniform = const_cast<xne_ShaderUniformDesc_t*>(&uniforms[0]);
    while (uniform->attrib != 0)
        uniform = const_cast<xne_ShaderUniformDesc_t*>(&uniforms[++shader->uniform_count]);

    if(!shader->uniform_count){
        return XNE_FAILURE;
    }
    
    shader->uniforms = (xne_Shader_t::xne_ShaderUniform*) realloc(shader->uniforms, sizeof(xne_Shader_t::xne_ShaderUniform) * shader->uniform_count);
    assert(shader->uniforms);
